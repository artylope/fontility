...
export function CustomFontDialog({ trigger, selectedFont, onFontSelect }: CustomFontDialogProps) {
  const [open, setOpen] = useState(false)
  const [fontFamily, setFontFamily] = useState('')
  const [fontFiles, setFontFiles] = useState<FontFile[]>([])
  const [dragOver, setDragOver] = useState(false)
  const [loading, setLoading] = useState(false)
  const [selectedFontId, setSelectedFontId] = useState<string>(selectedFont)
  const fileInputRef = useRef<HTMLInputElement>(null)
  const { customFonts, addCustomFont } = useFontPairStore()
  ...
  const handleSelectExisting = () => {
    const font = customFonts.find(f => f.family === selectedFontId)
    if (font) {
      onFontSelect(font)
      setOpen(false)
    }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="!w-full !max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Select Custom Font</DialogTitle>
        </DialogHeader>

        <div className="space-y-6 py-4">
          {/* Upload Section */}
          <div className="space-y-4">
            <h4 className="text-sm font-medium">Upload New Font</h4>
            ...
            >
              ...
              <p className="font-semibold">Drop font files here or click to browse</p>
              <p className="text-sm text-muted-foreground">Supports: {acceptedFormats.join(', ')}</p>
              <input
                ref={fileInputRef}
                type="file"
                multiple
          ...
          {customFonts.length > 0 && (
            <div className="space-y-4">
              <h4 className="text-sm font-medium">Your Custom Fonts</h4>
              <RadioGroup value={selectedFontId} onValueChange={setSelectedFontId}>
                  ...
                  {customFonts.map((font) => (
                    <div
                      ...
                      className={cn(
                        ...
                        selectedFontId === font.family ? "bg-muted border-primary" : "border-border"
                      )}
                      onClick={() => setSelectedFontId(font.family)}
                    >
                      <RadioGroupItem value={font.family} />
        ...
        <DialogFooter>
          ...
          {fontFiles.length > 0 ? (
            ...
            <Button
              ...
              disabled={!selectedFontId || !customFonts.find(f => f.family === selectedFontId)}
            >
              Select Font
            </Button>
          )}
